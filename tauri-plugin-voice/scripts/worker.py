...
# Disable XET transfer backend before importing huggingface_hub.
# hf-xet bypasses tqdm entirely, breaking download progress reporting.
...
def _make_progress_tqdm(model_repo):
    """Return a tqdm subclass that emits in_progress events per file.

    Only emits in_progress; the outer _load_model_with_progress handles
    the single start/complete lifecycle events for the download phase.
    Each tqdm instance corresponds to one file in the snapshot.
    """

    class _ProgressTqdm(tqdm.tqdm):
        """Wraps tqdm; serialises per-file download progress to stdout."""
        ...
        def update(self, n=1):
            ...
            # Fraction for this file; None when total is unknown.
...
def _load_model_with_progress(model_repo):
    ...
    # XET is disabled via env var at module top so tqdm_class is honoured.
