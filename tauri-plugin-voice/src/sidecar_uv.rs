...
impl UvSidecarRunner {
    pub async fn create_venv<R: Runtime>(
        ...
        while let Some(event) = rx.recv().await {
            match event {
                ...
                CommandEvent::Error(err) => {
                    return Err(Error::bootstrap_failed(format!(
                        "uv venv error: {}\nStderr: {}",
                        err,
                        stderr_lines.join("\n")
                    )));
        ...
        if !success {
            return Err(Error::bootstrap_failed(format!(
                "uv venv did not complete successfully. Stderr: {}",
                stderr_lines.join("\n")
            )));
        }
    ...
    pub async fn pip_install<R: Runtime>(
        ...
        let (mut rx, _child) = app
            ...
            .sidecar("uv")
            .map_err(|e| {
                Error::dependency_install_failed(format!("Failed to spawn uv sidecar: {}", e))
            })?
            .args([
                "pip",
                "install",
            ...
            .spawn()
            .map_err(|e| {
                Error::dependency_install_failed(format!("Failed to execute uv pip install: {}", e))
            })?;
        ...
        while let Some(event) = rx.recv().await {
            match event {
                ...
                CommandEvent::Error(err) => {
                    return Err(Error::dependency_install_failed(format!(
                        "uv pip install error: {}\nStderr: {}",
                        err,
                        stderr_lines.join("\n")
                    )));
                }
                CommandEvent::Terminated(payload) => {
                    if payload.code.unwrap_or(-1) == 0 {
                        ...
                        return Err(Error::dependency_install_failed(format!(
                            "uv pip install exited with code: {:?}\nStderr: {}",
                            payload.code,
                            stderr_lines.join("\n")
                        )));
                    }
        ...
        if !success {
            return Err(Error::dependency_install_failed(format!(
                "uv pip install did not complete successfully\nStderr: {}",
                stderr_lines.join("\n")
            )));
        }
